...
from materials import Material, PHYSICAL_CONSTANTS
...
class U2Engine:
    ...
    def __init__(self, material: Material, mesh: PressMesh, thickness_mm: float = 1.96):
        """
        Args:
            material: 材料オブジェクト
            mesh: メッシュオブジェクト
        ...
        # FLC₀を計算（初期板厚）
        self.flc0 = material.compute_FLC0(thickness_mm)
    ...
    def compute_U2_thermal(self) -> np.ndarray:
        ...
        m = self.material.data['mass']
        omega = self.material.omega
    ...
    def compute_U2_critical(self) -> np.ndarray:
        ...
        """
        return self.material.U2_critical(self.Z, self.T)
    ...
    def compute_lambda(self) -> np.ndarray:
        ...
        # FLC限界を計算（各頂点の板厚を使用）
        flc_limit = self.material.compute_FLC(self.thickness, self.strain_minor)
        
        # λ_strain = (|ε| / ε_limit)²
        lambda_strain = (np.abs(self.strain) / np.maximum(flc_limit, 0.01)) ** 2
        
        # 3. 表面効果（Z低→壊れやすい）- 穏やかに
        Z_bulk = self.material.data['Z_bulk']
    ...
    def _apply_heat_generation(self, collapsed_mask: np.ndarray):
        ...
        """
        k_B = PHYSICAL_CONSTANTS['k_B']
        bond_energy = self.material.bond_energy_J
